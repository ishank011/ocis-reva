...
func UsersWithConfig(cfg *config.Config) []cli.Flag {
	return []cli.Flag{
		...
		&cli.StringFlag{
			...
			EnvVars:     []string{"REVA_USERPROVIDER_NETWORK"},
			Destination: &cfg.Reva.UserProvider.Network,
		},
		&cli.StringFlag{
			Name:        "protocol",
			Value:       "grpc",
			Usage:       "protocol for reva service, can be 'http' or 'grpc'",
			EnvVars:     []string{"REVA_USERPROVIDER_PROTOCOL"},
			Destination: &cfg.Reva.UserProvider.Protocol,
		},
		&cli.StringFlag{
			Name:        "addr",
			Value:       "0.0.0.0:9144",
			Usage:       "Address to bind reva service",
			EnvVars:     []string{"REVA_USERPROVIDER_ADDR"},
			Destination: &cfg.Reva.UserProvider.Addr,
		},
		&cli.StringFlag{
			Name:        "url",
			Value:       "localhost:9144",
			Usage:       "URL to use for the reva service",
			EnvVars:     []string{"REVA_USERPROVIDER_URL"},
			Destination: &cfg.Reva.UserProvider.URL,
		},
		&cli.StringSliceFlag{
			Name:    "service",
		...
		&cli.StringFlag{
			...
			EnvVars:     []string{"REVA_USERPROVIDER_DRIVER"},
			Destination: &cfg.Reva.UserProvider.Driver,
		},
		&cli.StringFlag{
			Name:        "json-config",
			Value:       "",
			Usage:       "Path to users.json file",
			EnvVars:     []string{"REVA_USERPROVIDER_JSON"},
			Destination: &cfg.Reva.UserProvider.JSON,
		},
	}
}
